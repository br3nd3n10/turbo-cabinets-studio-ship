...
- **Pick a layout.** Choose **Longer boxes**. Run `control-studio browser click --selector "#template-list [data-template=longer]"`. `body` `data-phase` is `ready`. That card is pressed. `[data-mode=layout]` is pressed. `#job-download` is enabled.
- **Proof.** Snapshot welcome, sizing (no cards), templates (cards, no Layout), and ready (Layout pressed) under `.cursor/skills/verify-studio/artifacts/measure-first/`.
